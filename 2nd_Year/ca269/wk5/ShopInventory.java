...
class Bread extends Item implements StorageCondition{
     ...
     */
    Bread(String name, int expiry_days, double price) {
        super(name);
        this.expiresInDays = expiry_days;
        this.price = price;
    }
...
class Perfume extends Item implements SecureItem {
     ...
     * Sets locked to true.
     */
    Perfume(String name, int expiry_days, double price){
        super(name);
        this.expiresInDays = expiry_days;
        this.price = price;
        this.locked = true;
    ...
    public static double billItems(Item[] items, OnSale Sale) {
        ...
        for (Item item : items) {
            ...
            if (item instanceof Milk) {
                Milk storage = (Milk)item;
                String str_cond = storage.storageProcedure();
                System.out.println(item.name + " (" + item.price + ") " + "(Storage: " + str_cond + ")");   
            }
            else if (item instanceof Bread) {
                Bread storage = (Bread)item;
                String str_cond = storage.storageProcedure();
                System.out.println(item.name + " (" + item.price + ") " + "(Storage: " + str_cond + ")");   
            }
            else if (item instanceof Perfume) {
                Perfume storage = (Perfume)item;
                String str_cond = storage.storageProcedure();
                System.out.println(item.name + " (" + item.price + ") " + "(Storage: " + str_cond + ")");   
            }
            else{
        ...
        if (Sale != null) {
            if (Sale.saleCondition(items)) {
                reduced_total = Sale.reduction(total_amount);
                System.out.println("Actual Total: " + total_amount);
